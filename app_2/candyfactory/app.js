...
var check_roles = function(req, res, next) {
  ...
  switch(req.path) {
    case '/admin-menu':
      ...
      break;
    case '/admin-map':
      if (req.roles.includes('factory_ownerCY')) {
        next()
      } else {
        return res.render('notauthorized');
      }
    ...
    case '/admin-rooms':
      ...
      break;
    case '/candycaneroom':
      if (req.roles.includes('factory_ownerCY') || req.roles.includes('candycaneroom')) {
        next()
      } else {
        return res.render('notauthorized');
      }
      break;
    case '/chewinggumroom':
      if (req.roles.includes('factory_ownerCY') || req.roles.includes('chewinggumroom')) {
        next()
      } else {
        return res.render('notauthorized');
      }
      break;
    case '/candyroom':
      if (req.roles.includes('factory_ownerCY') || req.roles.includes('candyroom')) {
        next()
      } else {
        return res.render('notauthorized');
      }
...
app.get('/candycaneroom', verify_jwt, check_roles, function(req, res){
	res.render('televisionRoom', {username: req.userInfo.username});
});

// To render inventing room
app.get('/chewinggumroom', verify_jwt, check_roles, function(req, res){
  res.render('inventingRoom', {username: req.userInfo.username});
});

// To render chocolate room
app.get('/candyroom', verify_jwt, check_roles, function(req, res){
